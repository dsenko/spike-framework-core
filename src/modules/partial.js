...
app.partial = {
     ...
     * @param data
     */
    include: function(partial, data){
        app.debug('Invoke app.partial.include with params: {0} {1}', [partial, data]);

        app.partial[partial.__name] = $.extend(true, {}, app.controller.__dataArchive[partialObject.__name]);
        ...
        app.debug('Returning partial {0} template ', [partialObject.__name]);

        return partialObject.__template($.extend(true, partialObject, partialPassedData));
    },
    ...
    register: function (partialName, partialObjectOrExtending, partialObject) {
         ...
         */
        partialObject.render = function (selector, partialPassedData) {
            app.debug('Invoke partialObject.__render with params: {0} {1}', [selector, partialPassedData]);


            app.partial[partialObject.__name] = $.extend(true, {}, app.controller.__dataArchive[partialObject.__name]);
            ...
            selector.html(partialObject.__template($.extend(true, partialObject, partialPassedData)));
